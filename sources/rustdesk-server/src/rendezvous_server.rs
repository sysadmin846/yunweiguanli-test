...
enum Sink {
    ...
    Ws(WsSink, Option<Encrypt>),
}

impl Sink {
    fn set_encrypt(&mut self, enc: Encrypt) {
        match self {
            Sink::TcpStream(_, e) => *e = Some(enc),
            Sink::Ws(_, e) => *e = Some(enc),
        }
    }
}
...
impl RendezvousServer {
    ...
    pub async fn start(port: i32, serial: i32, key: &str, rmem: usize) -> ResultType<()> {
        ...
        // 启动 HTTP API 服务器（端口 = 主端口 + 3，即默认 21119）
        let http_api_pm = rs.pm.clone();
        let http_api_port = port;
        tokio::spawn(async move {
            if let Err(err) = crate::http_api::start_http_api(http_api_port, http_api_pm).await {
                log::error!("HTTP API server failed: {}", err);
            }
        });
        ...
        if let Ok(msg_in) = RendezvousMessage::parse_from_bytes(bytes) {
            match msg_in.union {
                ...
                Some(rendezvous_message::Union::RequestRelay(mut rf)) => {
                    ...
                    let conn_type = rf.conn_type.enum_value_or_default();
                    log::debug!(
                        "中继请求: id={} secure={} conn_type={:?} token_len={} licence_key_len={} ws={}",
                        rf.id,
                        rf.secure,
                        conn_type,
                        rf.token.len(),
    ...
    async fn handle_punch_hole_request(
        ...
        let mut ph = ph;
        let conn_type = ph.conn_type.enum_value_or_default();
        let nat_type = ph.nat_type.enum_value_or_default();
        log::debug!(
            "打洞请求: id={} conn_type={:?} nat_type={:?} token_len={} licence_key_len={} ws={}",
            ph.id,
            conn_type,
            nat_type,
            ph.token.len(),
        ...
        if let Some(peer) = self.pm.get(&id).await {
            ...
            let force_relay = ALWAYS_USE_RELAY.load(Ordering::SeqCst) || (peer_is_lan ^ is_lan);
            if force_relay {
                if peer_is_lan {
                    // https://github.com/rustdesk/rustdesk-server/issues/24
            ...
            let same_intranet: bool = !force_relay
                && !ws
                && (peer_is_lan && is_lan || {
            ...
            if let Ok(mut bytes) = msg.write_to_bytes() {
                match sink {
                    Sink::TcpStream(s, enc) => {
                        if let Some(enc) = enc {
                            let buf = enc.enc(&bytes);
                            allow_err!(s.send(Bytes::from(buf)).await);
                        } else {
                    ...
                    Sink::Ws(ws, enc) => {
                        if let Some(enc) = enc {
                            let buf = enc.enc(&bytes);
                            allow_err!(ws.send(tungstenite::Message::Binary(buf)).await);
                        } else {
    ...
    /// 发送 KeyExchange 消息以支持客户端的 secure_tcp 握手
    /// 返回生成的 box 私钥，用于后续解密客户端回复
    /// 仅当服务端配置了签名密钥 (sk) 时才发送
    #[inline]
    async fn send_key_exchange(
        ...
        // 仅当配置了签名密钥时才发送 KeyExchange
        let Some(ref sk) = self.inner.sk else {
            log::debug!("KeyExchange 跳过: 未配置签名密钥, addr={}", addr);
            return None;
        };
        ...
        log::debug!(
            "发送 KeyExchange: addr={}, ws={}, pk_len={}",
            addr, ws, our_pk_b.0.len()
        );
    ...
    /// 处理客户端的 KeyExchange 回复，完成 secure_tcp 握手
    /// 返回用于后续加密通信的 Encrypt 对象
    fn complete_secure_handshake(
        bytes: &[u8],
        our_sk_b: &box_::SecretKey,
        addr: SocketAddr,
    ) -> Option<Encrypt> {
        log::debug!("secure_tcp 握手: 收到消息, addr={}, len={}", addr, bytes.len());
        
        let msg_in = match RendezvousMessage::parse_from_bytes(bytes) {
            Ok(msg) => msg,
            Err(e) => {
                log::debug!("secure_tcp 握手: 无法解析消息, addr={}, err={}", addr, e);
                return None;
            }
        };
        ...
        match msg_in.union {
            Some(rendezvous_message::Union::KeyExchange(ex)) => {
                log::debug!("secure_tcp 握手: 收到 KeyExchange, keys 数量={}, addr={}", ex.keys.len(), addr);
                if ex.keys.len() != 2 {
                    log::debug!(
                        "secure_tcp 握手: 无效的 keys 数量 {}, addr={}",
                        ex.keys.len(), addr
                    );
                    return None;
                }
                ...
                log::debug!(
                    "secure_tcp 握手: their_pk_b len={}, encrypted_key len={}, addr={}",
                    their_pk_b.len(), encrypted_key.len(), addr
                );
                
                match Encrypt::decode(encrypted_key, their_pk_b, our_sk_b) {
                    Ok(symmetric_key) => {
                        log::info!("secure_tcp 握手完成: addr={}", addr);
                        Some(Encrypt::new(symmetric_key))
                    }
                    Err(e) => {
                        log::warn!("secure_tcp 握手: 解密失败, addr={}, err={}", addr, e);
                        None
                    }
                }
            }
            Some(other) => {
                // 不是 KeyExchange 消息，记录消息类型
                log::debug!("secure_tcp 握手: 收到非 KeyExchange 消息, addr={}, type={:?}", addr, std::mem::discriminant(&other));
                None
            }
            None => {
                log::debug!("secure_tcp 握手: 收到空消息, addr={}", addr);
                None
            }
        }
    }
    ...
    async fn handle_listener_inner(
        ...
        let mut sink;
        let mut encryptor: Option<Encrypt> = None;
        ...
        if ws {
            ...
            sink = Some(Sink::Ws(a, None));
            
            // 发送 KeyExchange 消息以支持 secure_tcp 握手
            let box_sk = self.send_key_exchange(&mut sink, addr, ws).await;
            
            while let Ok(Some(Ok(msg))) = timeout(30_000, b.next()).await {
                if let tungstenite::Message::Binary(bytes) = msg {
                    // 尝试完成 secure_tcp 握手（如果需要）
                    if encryptor.is_none() {
                        if let Some(ref sk) = box_sk {
                            if let Some(enc) = Self::complete_secure_handshake(&bytes, sk, addr) {
                                encryptor = Some(enc.clone());
                                if let Some(s) = sink.as_mut() {
                                    s.set_encrypt(enc);
                                }
                                continue; // 握手消息已处理，继续等待下一条消息
                            }
                        }
                    }
                    
                    // 解密消息（如果已建立加密通道）
                    let decrypted = if let Some(ref mut enc) = encryptor {
                        let mut buf = BytesMut::from(&bytes[..]);
                        if enc.dec(&mut buf).is_err() {
                            log::warn!("消息解密失败, addr={}", addr);
                            break;
                        }
                        buf.to_vec()
                    } else {
                    ...
                    if !self.handle_tcp(&decrypted, &mut sink, addr, key, ws).await {
                        break;
                    }
                }
            }
